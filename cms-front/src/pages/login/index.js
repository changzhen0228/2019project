...
class LoginForm extends Component {
  ...
  handleWebsiteChange = value => {
    ...
    if (value) {
      //baidu
      autoCompleteResult = [".com", ".cn", "org"].map(
        domain => `${value}${domain}`
      );
    }
        ...
        <Form onSubmit={handleSubmit} style={{ width: "500px" }}>
          ...
          <FormItem label="用户名" {...formItemlayout}>
            {getFieldDecorator("username", {
              rules: [
                ...
                { min: 1, message: "用户名最少输入6位" },
                { max: 8, message: "用户名最大输入8位" }
              ]
            })(
              <Input
                prefix={
                  <Icon type="user" style={{ color: "rbga(0,0,0,.25)" }} />
                }
                placeholder="请输入用户名"
              />
            )}
          ...
          <FormItem label="密码" {...formItemlayout}>
            {getFieldDecorator("password", {
              ...
              <Input
                prefix={
                  <Icon type="lock" style={{ color: "rbga(0,0,0,.25)" }} />
                }
                placeholder="请输入密码"
              />
            )}
          ...
          {!isLogin && (
            <FormItem label="邮箱" {...formItemlayout}>
              {getFieldDecorator("email", {
                ...
                <Input
                  prefix={
                    <Icon type="mail" style={{ color: "rbga(0,0,0,.25)" }} />
                  }
                  placeholder="邮箱"
                />
              )}
          ...
          {!isLogin && (
            <FormItem label="手机号" {...formItemlayout}>
              {getFieldDecorator("phone", {
                ...
                <Input
                  prefix={
                    <Icon type="phone" style={{ color: "rbga(0,0,0,.25)" }} />
                  }
                  placeholder="请输入手机号"
                />
              )}
          ...
          {isLogin ? (
            <FormItem>
              ...
              没有账号?
              <a href="#" onClick={switchLoginStatus}>
                立即注册
              </a>
            </FormItem>
          ) : (
